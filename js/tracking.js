...
document.getElementById('trackingForm').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        const response = await fetch('api/track_shipment.php', {
            method: 'POST',
            body: formData
        });
        ...
        document.getElementById('searchButton').innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>Track';
    }
});
...
function showTrackingResults(shipment) {
    ...
    let statusInEnglish = shipment.status;
    switch(shipment.status) {
        case 'In Transit':
            statusInEnglish = 'In Transit';
    ...
    sortedUpdates.forEach((update, index) => {
        ...
        // Translate status to English
        let statusInEnglish = update.status;
        ...
        switch(update.status) {
            ...
            case 'Shipment Picked Up':
                statusInEnglish = 'Shipment Picked Up';
        ...
        timelineItem.innerHTML = `
            ...
            <div class="ml-4 flex-grow bg-gray-50 dark:bg-gray-800 p-4 rounded-lg shadow-sm">
                <div class="flex items-center justify-between mb-2">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">${statusInEnglish}</h3>
    ...
    addMarker(
        ...
        '<div class="p-2"><strong>Current Location:</strong><br>' + shipment.current_location + '</div>'
    );
    addMarker(
        destination, 
        'Destination', 
        ...
        '<div class="p-2"><strong>Destination:</strong><br>' + shipment.destination + '</div>'
    );
