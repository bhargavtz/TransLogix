...
<body class="bg-gray-50 dark:bg-gray-900 transition-colors duration-200">
    ...
    <main class="ml-64 p-8">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-4">Contact Messages</h1>
                        ...
                        <?php if ($result->num_rows > 0): ?>
                            <?php while ($row = $result->fetch_assoc()): ?>
                                <tr class="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700">
                                    ...
                                    <td class="py-3 px-6"><?php echo htmlspecialchars($row['subject']); ?></td>
                                    <td class="py-3 px-6"><?php echo htmlspecialchars($row['message']); ?></td>
                                    <td class="py-3 px-6"><?php echo htmlspecialchars($row['created_at']); ?></td>
                                    <td class="py-3 px-6 text-center">
                                        <a href="delete-contact.php?id=<?php echo $row['id']; ?>" class="text-red-500 hover:underline" onclick="return confirm('Are you sure you want to delete this message?');">Delete</a>
                                    </td>
                                </tr>
                            <?php endwhile; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="7" class="py-3 px-6 text-center">No messages found</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
